...
// queries are commands that just return strings and don't affect anything in
// the world
export const queryList = [
  {
    ...
    perform: function(agent) {
      ...
      res += `\n- Position: x: ${pos.x.toFixed(2)}, y: ${
          pos.y.toFixed(2)}, z: ${pos.z.toFixed(2)}`;
      // Gameplay
    ...
    perform: function(agent) {
      ...
      if (block_details.size === 0) {
        ...
        res += `\n- First Solid Block Above Head: ${
            world.getFirstBlockAboveHead(bot, null, 32)}`;
      }
      return pad(res);
    }
  },
  ...
  },
  {
    ...
    perform: function(agent) {
      ...
      let babyVillagerIds = [];
      let villagerDetails =
          [];  // Store detailed villager info including profession
      ...
      for (const entity of nearbyEntities) {
        ...
        if (entity.name === 'villager') {
          if (entity.metadata && entity.metadata[16] === 1) {
            ...
            villagerDetails.push({id: entity.id, profession: profession});
          }
        }
      }
      ...
      for (const [entityType, count] of Object.entries(entityCounts)) {
        if (entityType === 'villager') {
          ...
          if (villagerDetails.length > 0) {
            const detailStrings =
                villagerDetails.map(v => `(${v.id}:${v.profession})`);
            villagerInfo += ` - Adults: ${detailStrings.join(', ')}`;
          }
          if (babyVillagerIds.length > 0) {
            villagerInfo += ` - Baby IDs: ${
                babyVillagerIds.join(', ')} (babies cannot trade)`;
          }
  ...
  },
  {
    ...
    params: {
      targetItem:
          {type: 'string', description: 'The item that we are trying to craft'},
      quantity: {
        type: 'int',
    ...
    perform: function(agent, targetItem, quantity = 1) {
      ...
      if (existingCount > 0) {
        ...
        prefixMessage = `You already have ${existingCount} ${
            target_item} in your inventory. If you need to craft more,\n`;
      }

      // Generate crafting plan
      try {
        let craftingPlan =
            mc.getDetailedCraftingPlan(target_item, quantity, curr_inventory);
        craftingPlan = prefixMessage + craftingPlan;
        return pad(craftingPlan);
      } catch (error) {
        console.error('Error generating crafting plan:', error);
        return `An error occurred while generating the crafting plan: ${
            error.message}`;
      }
    },
  },
    ...
    perform: async function(agent, query) {
      ...
      try {
        ...
        if (response.status === 404) {
          return `${
              query} was not found on the Minecraft Wiki. Try adjusting your search term.`;
        }
