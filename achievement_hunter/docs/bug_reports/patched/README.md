...
| 6 | `BUG6_drowning_during_llm_call.md` | High | `ah_modes.js:87–91` (`blockAbove === 'water'` jump) — Fix A only; Fix B (`ensure_safe_before_llm`) removed on `search-agent`, see BUG 9 |
...
| 9 | `BUG9_goalchanged_crashes_spl_at_recovery_boundary.md` | High | **Patch removed** on `search-agent` — `ensure_safe_before_llm` deleted entirely (owner determined the underlying check was ineffective); race condition described in the report no longer exists because the racing `moveAway` call is gone |
