...
function make_models(
    ptd_responses, feedback_responses, refinement_responses = null) {
  return {
  ...
  vertices: [
    {
      ...
      qty: 1,
      item_type: 'item',
      acquisition_dependency: 'craft'
    },
    {
      id: 'oak_log',
      qty: 3,
      item_type: 'resource',
      acquisition_dependency: 'mine'
    },
  ],
  edges: [
    {
...
describe('generate_self_refined_ptd', () => {
  it('calls send_prompt at least once with a non-empty string', async () => {
    ...
    await generate_self_refined_ptd(
        models, 'craft a wooden pickaxe', null, null, DEFAULT_OPTS);
  ...
  it('embeds the task name in the initial prompt', async () => {
    ...
    await generate_self_refined_ptd(
        models, task_name, null, null, DEFAULT_OPTS);
    const [prompt] = models.ptd.send_prompt.mock.calls[0];
  ...
  it('returns ok:true with graph and trace when validator passes', async () => {
    ...
    const result = await generate_self_refined_ptd(
        models, 'mine 10 diamonds', null, null, DEFAULT_OPTS);
  ...
  it('returns ok:false when validator always fails within max_rounds',
     async () => {
       const models = make_models(
           [VALID_GRAPH, VALID_GRAPH], [FAIL_VERDICT, FAIL_VERDICT]);
       const result = await generate_self_refined_ptd(
           models, 'mine 10 diamonds', null, null, DEFAULT_OPTS);
  ...
  it('skips generation and returns existing graph when one is provided',
     async () => {
       const existing = JSON.parse(VALID_GRAPH);
       const models = make_models([VALID_GRAPH], [PASS_VERDICT]);
       const result = await generate_self_refined_ptd(
           models, 'craft a wooden pickaxe', existing, null, DEFAULT_OPTS);
       expect(result.ok).toBe(true);
