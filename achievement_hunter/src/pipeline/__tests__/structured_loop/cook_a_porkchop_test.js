 ...
 * Every AM command string expectation matches the exact value logged in that
 * rollout.
 *
 * Rollout stage summary:
 *   1. collect any_log (spruce_log nearby)     → !collectBlocks("spruce_log",
 * 3)
 *   2. craft spruce_planks (3 logs, batch=4)   → !craftRecipe("spruce_planks",
 * 3)
 *   3. craft stick (2 needed, batch=4)         → !craftRecipe("stick", 1)
 *   4. craft crafting_table                    → !craftRecipe("crafting_table",
 * 1)
 *   5. craft wooden_pickaxe                    → !craftRecipe("wooden_pickaxe",
 * 1)
 *   6. collect cobblestone (stone nearby)      → !collectBlocks("stone", 8)
 *   7. craft furnace                           → !craftRecipe("furnace", 1)
 *   8. kill pig (search then attack)           → !search("pig") →
 * !attack("pig")
 *   9. smelt porkchop (any_plank → spruce_planks) → !smelt_item("porkchop", 1,
 * "spruce_planks")
 */
...
// State at stage 5: has planks + sticks + crafting_table, wooden_pickaxe
// craftable
const STATE_STAGE_5 = {
  craftable_items: [
    'spruce_planks',
...
const STATE_STAGE_7 = {
  craftable_items: [
    'spruce_planks',  'spruce_slab',        'cobblestone_slab',
    'furnace',        'cobblestone_stairs', 'cobblestone_wall',
    'lever',          'spruce_button',      'spruce_pressure_plate',
    'bowl',           'wooden_sword',       'wooden_shovel',
    'wooden_pickaxe', 'wooden_axe',         'wooden_hoe',
    'stone_sword',    'stone_shovel',       'stone_pickaxe',
    'stone_axe',      'stone_hoe',          'stick',
  ],
  nearby_blocks: [
    'stone',
    'gravel',
...
describe('check_search_complete', () => {
  ...
  it('returns false when stone is not in nearby_blocks (stage 1 has stone but checking a missing block)',
     () => {
       const state = {
         nearby_blocks: ['dirt', 'grass_block'],
         nearby_entities: {mobs: []}
       };
  ...
  it('returns false when state has no nearby_entities', () => {
    expect(
        check_search_complete('pig', {nearby_blocks: [], nearby_entities: {}}))
        .toBe(false);
  });

  it('returns false when state has no nearby_blocks', () => {
    expect(check_search_complete(
               'oak_log', {nearby_blocks: [], nearby_entities: {mobs: []}}))
        .toBe(false);
  });
});
...
describe('make_search_command', () => {
  it('generates searchForEntity for pig at radius 32 (rollout stage 8 search)',
     () => {
       expect(make_search_command('pig', 32))
           .toBe('!searchForEntity("pig", 32)');
     });

  it('generates searchForEntity for pig at radius 64 (rollout stage 8 second attempt)',
     () => {
       expect(make_search_command('pig', 64))
           .toBe('!searchForEntity("pig", 64)');
     });

  it('generates searchForBlock for spruce_log at radius 32', () => {
    expect(make_search_command('spruce_log', 32))
        .toBe('!searchForBlock("spruce_log", 32)');
  });

  it('generates searchForBlock for stone at radius 128', () => {
    expect(make_search_command('stone', 128))
        .toBe('!searchForBlock("stone", 128)');
  });
});
...
describe('expand_search_item', () => {
  ...
  it('throws for an unsupported abstract target like "any_plank"', () => {
    expect(() => expand_search_item('any_plank'))
        .toThrow('Unsupported abstract search target');
  });
});
...
describe('is_successful_command_result', () => {
  it('returns true for a non-string result (object)', () => {
    expect(is_successful_command_result({
      success: true,
  ...
  it('returns true for a clean Action output result object', () => {
    expect(is_successful_command_result({
      success: true,
      message: 'Action output: collected 3 spruce_log'
    })).toBe(true);
  });

  it('returns false for a string containing "Error:"', () => {
    expect(is_successful_command_result(
               'Action output: Error: could not reach block'))
        .toBe(false);
  });

  it('returns false for a string containing "Could not find"', () => {
    expect(is_successful_command_result(
               'Action output: Could not find spruce_log'))
        .toBe(false);
  });

  it('returns false for a string containing "!!Code threw exception!!"', () => {
    expect(
        is_successful_command_result('Action output: !!Code threw exception!!'))
        .toBe(false);
  });
...
describe('get_command_failure_signature', () => {
  ...
  it('normalizes different timeout durations to the same signature', () => {
    ...
    const result_a = {
      success: false,
      message:
          'Action output: Error: Event updateSlot:0 did not fire within timeout 250ms'
    };
    const result_b = {
      success: false,
      message:
          'Action output: Error: Event updateSlot:0 did not fire within timeout 300ms'
    };
    expect(get_command_failure_signature(cmd, result_a))
        .toBe(
  ...
  it('produces different signatures for different commands', () => {
    const result = {
      success: false,
      message: 'Action output: Error: Could not find'
    };
    const sig_a =
        get_command_failure_signature('!craftRecipe("stick", 1)', result);
    const sig_b =
        get_command_failure_signature('!craftRecipe("furnace", 1)', result);
...
describe('is_craft_command', () => {
  ...
  it('returns true for !smelt_item(...) — rollout stage 9', () => {
    expect(is_craft_command('!smelt_item("porkchop", 1, "spruce_planks")'))
        .toBe(true);
  });
...
describe('resolve_nearby_block_source', () => {
  it('resolves any_log → spruce_log when spruce_log is nearby (rollout stage 1)',
     () => {
       expect(resolve_nearby_block_source(
                  CANDIDATE_ANY_LOG, STATE_STAGE_1.nearby_blocks))
           .toBe('spruce_log');
     });
  ...
  it('resolves cobblestone → stone when stone is nearby (rollout stage 6)',
     () => {
       expect(resolve_nearby_block_source(
                  CANDIDATE_COBBLESTONE, STATE_STAGE_6.nearby_blocks))
           .toBe('stone');
     });

  it('returns null for porkchop — it has no block source', () => {
    expect(resolve_nearby_block_source(
               CANDIDATE_PORKCHOP, STATE_STAGE_1.nearby_blocks))
        .toBeNull();
  });

  it('returns null for any_log when no log types are nearby', () => {
    expect(resolve_nearby_block_source(CANDIDATE_ANY_LOG, [
  ...
  it('resolves water_bucket → water when water is nearby', () => {
    ...
    expect(resolve_nearby_block_source(candidate, [
      'water', 'stone'
    ])).toBe('water');
  });
...
describe('resolve_fallback_block_source', () => {
  it('falls back to "stone" for cobblestone when nothing nearby', () => {
    expect(resolve_fallback_block_source(CANDIDATE_COBBLESTONE, []))
        .toBe('stone');
  });

  it('falls back to "any_log" for any_log when no concrete log is nearby',
     () => {
       expect(resolve_fallback_block_source(CANDIDATE_ANY_LOG, [
         'dirt', 'grass_block'
       ])).toBe('any_log');
     });
...
describe('resolve_nearby_mob_source', () => {
  it('resolves porkchop candidate → pig when pig is in nearby_mobs (rollout stage 8 attempt 2)',
     () => {
       expect(resolve_nearby_mob_source(CANDIDATE_PORKCHOP, [
         'pig', 'chicken'
       ])).toBe('pig');
     });

  it('returns null for porkchop when only chicken is nearby', () => {
    expect(resolve_nearby_mob_source(CANDIDATE_PORKCHOP, ['chicken']))
        .toBeNull();
  });
  ...
  it('returns null for cobblestone — has no mob source', () => {
    expect(resolve_nearby_mob_source(CANDIDATE_COBBLESTONE, [
      'pig', 'cow'
    ])).toBeNull();
  });
});
...
describe('get_satisfied_inputs_by_type', () => {
  it('extracts crafting_inputs from wooden_pickaxe candidate (rollout stage 5)',
     () => {
       const result = get_satisfied_inputs_by_type(
           CANDIDATE_WOODEN_PICKAXE, 'crafting_input');
  ...
  it('returns empty array when no inputs match the type', () => {
    expect(get_satisfied_inputs_by_type(CANDIDATE_ANY_LOG, 'crafting_input'))
        .toEqual([]);
  });
...
describe('get_single_satisfied_input_item', () => {
  it('returns "crafting_table" as workstation for wooden_pickaxe (rollout stage 5)',
     () => {
       expect(get_single_satisfied_input_item(
                  CANDIDATE_WOODEN_PICKAXE, 'workstation_dependency'))
           .toBe('crafting_table');
     });
  ...
  it('returns "furnace" as workstation for cooked_porkchop (rollout stage 9)',
     () => {
       expect(get_single_satisfied_input_item(
                  CANDIDATE_COOKED_PORKCHOP, 'workstation_dependency'))
           .toBe('furnace');
     });

  it('returns null when the type is not present', () => {
    expect(get_single_satisfied_input_item(
               CANDIDATE_ANY_PLANK, 'workstation_dependency'))
        .toBeNull();
  });

  it('returns null for porkchop candidate with empty satisfied_inputs', () => {
    expect(
        get_single_satisfied_input_item(CANDIDATE_PORKCHOP, 'tool_dependency'))
        .toBeNull();
  });
  ...
  it('returns "wooden_pickaxe" as tool for cobblestone candidate (rollout stage 6)',
     () => {
       expect(get_single_satisfied_input_item(
                  CANDIDATE_COBBLESTONE, 'tool_dependency'))
           .toBe('wooden_pickaxe');
     });
});
...
describe('resolve_concrete_craft_target', () => {
  it('resolves any_plank → spruce_planks when spruce_planks is craftable (rollout stage 2)',
     () => {
       expect(resolve_concrete_craft_target('any_plank', ['spruce_planks']))
           .toBe('spruce_planks');
     });

  it('resolves concrete "stick" when stick is craftable (rollout stage 3)',
     () => {
       expect(resolve_concrete_craft_target(
                  'stick', STATE_STAGE_2.craftable_items.concat(['stick'])))
           .toBe('stick');
     });

  it('resolves "crafting_table" when craftable (rollout stage 4)', () => {
    expect(resolve_concrete_craft_target('crafting_table', [
  ...
  it('resolves "wooden_pickaxe" when craftable (rollout stage 5)', () => {
    expect(resolve_concrete_craft_target(
               'wooden_pickaxe', STATE_STAGE_5.craftable_items))
        .toBe('wooden_pickaxe');
  });

  it('resolves "furnace" when craftable (rollout stage 7)', () => {
    expect(
        resolve_concrete_craft_target('furnace', STATE_STAGE_7.craftable_items))
        .toBe('furnace');
  });

  it('returns null when item is not in craftable_items', () => {
    expect(resolve_concrete_craft_target('furnace', [
  ...
  it('returns null when any_plank has no matching member in craftable_items',
     () => {
       expect(resolve_concrete_craft_target('any_plank', [
         'furnace', 'stick'
       ])).toBeNull();
     });
});
...
describe('try_make_craft_task', () => {
  it('builds craft task for any_plank → spruce_planks (rollout stage 2 NTS)',
     () => {
       const task = try_make_craft_task(
           CANDIDATE_ANY_PLANK, {craftable_items: ['spruce_planks']});
       expect(task).toEqual({
  ...
  it('builds craft task for stick with no workstation (rollout stage 3 NTS)',
     () => {
       const task =
           try_make_craft_task(CANDIDATE_STICK, {craftable_items: ['stick']});
       expect(task).toEqual({
         target_item: 'stick',
         qty: 2,
         action_type: 'craft',
  ...
  it('builds craft task for wooden_pickaxe with crafting_table workstation (rollout stage 5)',
     () => {
       const task = try_make_craft_task(
           CANDIDATE_WOODEN_PICKAXE,
           {craftable_items: STATE_STAGE_5.craftable_items});
       expect(task).toEqual({
         ...
         parameters: {
           crafting_inputs:
               [{item: 'stick', qty: 2}, {item: 'any_plank', qty: 3}],
  ...
  it('builds craft task for furnace with crafting_table workstation (rollout stage 7)',
     () => {
       const task = try_make_craft_task(
           CANDIDATE_FURNACE, {craftable_items: STATE_STAGE_7.craftable_items});
       expect(task).toEqual({
         target_item: 'furnace',
         qty: 1,
         action_type: 'craft',
  ...
  it('returns null when the item is not yet craftable', () => {
    expect(try_make_craft_task(CANDIDATE_ANY_PLANK, {
      craftable_items: []
    })).toBeNull();
  });

  it('returns null for a resource candidate (porkchop is not itemish)', () => {
    expect(try_make_craft_task(CANDIDATE_PORKCHOP, {
      craftable_items: ['porkchop']
    })).toBeNull();
  });
});
...
describe('try_make_immediate_acquisition_task', () => {
  it('builds kill task for porkchop when pig is nearby (rollout stage 8 attempt 2)',
     () => {
       const task = try_make_immediate_acquisition_task(
           CANDIDATE_PORKCHOP, STATE_PIG_FOUND);
       expect(task).toEqual({
  ...
  it('returns null for porkchop when pig is not nearby (rollout stages 1-7)',
     () => {
       expect(try_make_immediate_acquisition_task(
                  CANDIDATE_PORKCHOP, STATE_STAGE_1))
           .toBeNull();
     });
  ...
  it('builds collect task for any_log when spruce_log is nearby (rollout stage 1)',
     () => {
       const task = try_make_immediate_acquisition_task(
           CANDIDATE_ANY_LOG, STATE_STAGE_1);
       expect(task).toEqual({
         target_item: 'any_log',
         qty: 3,
         action_type: 'collect',
         parameters:
             {source_block: 'spruce_log', item_dependency: null, tool: null},
       });
     });
  ...
  it('builds collect task for cobblestone with tool=wooden_pickaxe (rollout stage 6)',
     () => {
       const task = try_make_immediate_acquisition_task(
           CANDIDATE_COBBLESTONE, STATE_STAGE_6);
       expect(task).toEqual({
  ...
  it('returns null for cobblestone when stone is not nearby', () => {
    ...
    expect(try_make_immediate_acquisition_task(CANDIDATE_COBBLESTONE, no_stone))
        .toBeNull();
  });
});
...
describe('make_fallback_acquisition_task', () => {
  it('builds fallback kill task for porkchop using canonical mob pig', () => {
    const task =
        make_fallback_acquisition_task(CANDIDATE_PORKCHOP, {nearby_blocks: []});
    expect(task).toEqual({
      target_item: 'porkchop',
      qty: 1,
      action_type: 'kill',
  ...
  it('builds fallback collect task for cobblestone using canonical source stone',
     () => {
       const task = make_fallback_acquisition_task(
           CANDIDATE_COBBLESTONE, {nearby_blocks: []});
       expect(task).toEqual({
         target_item: 'cobblestone',
         qty: 8,
         action_type: 'collect',
  ...
  it('builds fallback collect for any_log using "any_log" as source when nothing nearby',
     () => {
       const task = make_fallback_acquisition_task(
           CANDIDATE_ANY_LOG, {nearby_blocks: []});
...
describe('select_next_task', () => {
  ...
  it('stage 3: picks craft stick (now craftable)', () => {
    const state = {
      ...STATE_STAGE_2,
      craftable_items:
          [...STATE_STAGE_2.craftable_items, 'stick', 'crafting_table']
    };
    const task = select_next_task(
        [CANDIDATE_PORKCHOP, CANDIDATE_STICK, CANDIDATE_CRAFTING_TABLE], state);
    expect(task.action_type).toBe('craft');
  ...
  it('returns null when no candidate can produce a task', () => {
    ...
    expect(select_next_task([unsatisfiable], {
      craftable_items: [],
      nearby_blocks: [],
      nearby_entities: {mobs: []}
    })).toBeNull();
  ...
  it('craft tier takes priority over immediate acquisition (craft beats nearby block)',
     () => {
       ...
       const task =
           select_next_task([CANDIDATE_ANY_LOG, CANDIDATE_ANY_PLANK], state);
       expect(task.action_type).toBe('craft');
     });
});
...
describe('mediate_collect', () => {
  ...
  const TASK_COLLECT_COBBLESTONE = {
    ...
    action_type: 'collect',
    parameters:
        {source_block: 'stone', item_dependency: null, tool: 'wooden_pickaxe'},
  };
  ...
  it('generates !collectBlocks("spruce_log", 3) when spruce_log is nearby (rollout stage 1 AM)',
     () => {
       ...
       expect(action).toEqual(
           {kind: 'command', command: '!collectBlocks("spruce_log", 3)'});
     });
  ...
  it('generates !collectBlocks("stone", 8) when stone is nearby (rollout stage 6 AM)',
     () => {
       ...
       expect(action).toEqual(
           {kind: 'command', command: '!collectBlocks("stone", 8)'});
     });
  ...
  it('generates !useOn for water + item_dependency (bucket fill)', () => {
    const task = {
      ...
      action_type: 'collect',
      parameters:
          {source_block: 'water', item_dependency: 'bucket', tool: null},
    };
    const action = mediate_collect(task, {nearby_blocks: ['water']});
    expect(action).toEqual(
        {kind: 'command', command: '!useOn("bucket", "water")'});
  });
...
describe('mediate_craft', () => {
  ...
  it('generates !craftRecipe("spruce_planks", 3) — qty=10, batch=4, ceil(10/4)=3 (rollout stage 2 AM)',
     () => {
       ...
       expect(mediate_craft(task).command)
           .toBe('!craftRecipe("spruce_planks", 3)');
     });
  ...
  it('generates !craftRecipe("stick", 1) — qty=2, batch=4, ceil(2/4)=1 (rollout stage 3 AM)',
     () => {
       vi.mocked(get_item_batch_size).mockReturnValue(4);
       const task =
           {target_item: 'stick', qty: 2, action_type: 'craft', parameters: {}};
  ...
  it('generates !craftRecipe("crafting_table", 1) — qty=1, batch=1 (rollout stage 4 AM)',
     () => {
       ...
       expect(mediate_craft(task).command)
           .toBe('!craftRecipe("crafting_table", 1)');
     });
  ...
  it('generates !craftRecipe("wooden_pickaxe", 1) — qty=1, batch=1 (rollout stage 5 AM)',
     () => {
       ...
       expect(mediate_craft(task).command)
           .toBe('!craftRecipe("wooden_pickaxe", 1)');
     });
...
describe('resolve_smelt_fuel_name', () => {
  ...
  it('resolves any_plank → spruce_planks when spruce_planks is in inventory (rollout stage 9)',
     () => {
       expect(resolve_smelt_fuel_name(SMELT_TASK, STATE_SMELT))
           .toBe('spruce_planks');
     });
  ...
  it('resolves a concrete fuel item directly from inventory', () => {
    ...
    expect(resolve_smelt_fuel_name(concrete_task, {
      inventory: {coal: 3}
    })).toBe('coal');
  });
...
describe('mediate_smelt', () => {
  ...
  it('throws when smelting_inputs is empty', () => {
    const bad_task = {
      ...SMELT_TASK,
      parameters: {...SMELT_TASK.parameters, smelting_inputs: []}
    };
    expect(() => mediate_smelt(bad_task, STATE_SMELT))
        .toThrow('Smelt task missing smelting_inputs');
  });
});
